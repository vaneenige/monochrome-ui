...
on, and the wrappers never work out which one that is.
...
applied after them, so the id, role, and aria attributes stay
the wrapper's. This is the contract plain HTML already has, so
one menubar reads the same in all three renderers. From then on
the core moves the stop to whichever bar item takes focus (see
`docs/menu.md`, "Moving tab stop"), as Tabs does for the
selected tab. React and Vue only write a prop again when its
value changes, so a re-render leaves the moved stop alone; an
author who changes the `tabIndex` they pass moves it back. The
cost of an authored contract is that a bar whose items are all
`-1` has no tab stop and is skipped by Tab entirely. Nothing
warns about it.
