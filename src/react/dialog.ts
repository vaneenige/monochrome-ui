...
function Content({ children, ...props }: BaseProps): ReactElement {
	...
	return createElement(
		"dialog",
		{
			...(hasLabel
				? {}
				: { "aria-labelledby": `mcc:dialog-title:${context.id}` }),
			...(hasDescription
				? {}
				: { "aria-describedby": `mcc:dialog-description:${context.id}` }),
			...props,
...
function Title({
	...
	return createElement(
		as ?? "h2",
		{ ...props, id: `mcc:dialog-title:${context.id}` },
		children,
	);
}
...
function Description({ children, ...props }: BaseProps): ReactElement {
	...
	return createElement(
		"p",
		{ ...props, id: `mcc:dialog-description:${context.id}` },
		children,
	);
}
