		...
		return () => {
			...
			return h(
				"dialog",
				{
					...(hasLabel
						? {}
						: { "aria-labelledby": `mcc:dialog-title:${ctx.id}` }),
					...(hasDescription
						? {}
						: { "aria-describedby": `mcc:dialog-description:${ctx.id}` }),
					...attrs,
...
const Title = defineComponent({
	...
	setup(props, { slots }) {
		...
		return () =>
			h(props.as, { id: `mcc:dialog-title:${ctx.id}` }, slots.default?.());
	},
});
...
const Description = defineComponent({
	setup(_, { slots }) {
		...
		return () =>
			h("p", { id: `mcc:dialog-description:${ctx.id}` }, slots.default?.());
	},
});
