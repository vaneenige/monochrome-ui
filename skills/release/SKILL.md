...
6. Run the full gate and stop on any failure: `npm run build` (also lints), then `npm run typecheck` (needs the `dist/` the build just produced), then `npm run test`.
7. Bump without tagging: `npm version <bump> --no-git-tag-version`, then read the new version from package.json.
8. Show what will ship — the package name, the new version, and the output of `npm pack --dry-run` — then ask: "Publish this version, or skip?" Wait for the answer; do nothing further unless they say publish.
...
10. Publish — the point of no return, a version can never be re-published: `npm publish --access public --ignore-scripts --//registry.npmjs.org/:_authToken=$NPM_TOKEN`. If it fails, stop and undo the still-local commit and tag (`git reset --hard HEAD~1 && git tag -d v<version>`); nothing has been pushed.
