...
test.describe("Router", () => {
	...
	test.describe("Root swap", () => {
		...
		test("never replaces the body or head", async ({ page }) => {
			...
			await expect(page).toHaveURL("/html/router/about");
			expect(
				await page.locator("body").getAttribute("data-preserved-body"),
			).toBe("yes");
			expect(
				await page.locator("head").getAttribute("data-preserved-head"),
			).toBe("yes");
		});
		...
		test("preserves siblings outside the root area", async ({ page }) => {
			...
			await page.getByTestId("nav-about").click();
			await expect(page).toHaveURL("/html/router/about");
			expect(await page.locator("nav").getAttribute("data-preserved")).toBe(
				"yes",
			);
		});
		...
		test("swaps `data-area` elements in the head during a root swap", async ({
			...
			await page.goto("/html/router/index");
			await expect(page.locator("meta[data-area='head-meta']")).toHaveAttribute(
				"content",
				"index",
			);
			await page.getByTestId("nav-about").click();
	...
	test.describe("Area keys", () => {
		test("preserves an area whose `data-key` matches across pages", async ({
			...
			await page.evaluate(() => {
				document
					.querySelector("[data-area='sidebar']")
			...
			expect(
				await page
					.locator("[data-area='sidebar']")
					.getAttribute("data-preserved"),
			).toBe("yes");
		});
	...
	test.describe("History", () => {
		...
		test("restores scroll position on back navigation", async ({ page }) => {
			...
			await page.evaluate(() => {
				document
					.querySelector<HTMLAnchorElement>("[data-testid='link-next']")
					?.click();
			});
	...
	test.describe("Prefetching", () => {
		...
		test("prefetches a link on hover", async ({ page }) => {
			...
			await expect
				.poll(() => fetched.some((u) => u.endsWith("/html/router/about")))
				.toBe(true);
		});
	...
	test.describe("Concurrent navigation", () => {
		test("no-ops a click on the current URL", async ({ page }) => {
			...
			await page.evaluate(() => {
				document
					.querySelector("[data-area='root']")
					?.setAttribute("data-preserved", "yes");
			});
			await page.getByTestId("nav-home").click();
			await page.waitForTimeout(80);
			expect(
				await page.locator("[data-area='root']").getAttribute("data-preserved"),
			).toBe("yes");
		});
		...
		test("drops a stale in-flight navigation when a later one arrives", async ({
			...
			await page.evaluate(() => {
				document
					.querySelector<HTMLAnchorElement>("[data-testid='nav-about']")
					?.click();
			});
			await page.waitForTimeout(50);
			await page.evaluate(() => {
				document
					.querySelector<HTMLAnchorElement>("[data-testid='nav-docs']")
					?.click();
			});
			await expect(page).toHaveURL("/html/router/docs");
			await expect(page.getByTestId("page-title")).toHaveText("Docs");
