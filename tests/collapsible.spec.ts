...
test.describe("Collapsible", () => {
	test.describe("ARIA Attributes", () => {
		...
		test("should have `aria-expanded='false'` when closed", async ({
			page,
			renderer,
		}) => {
		...
		test("should have `aria-expanded='true'` when open", async ({
			page,
			renderer,
		}) => {
		...
		test("should have `aria-controls` on trigger linking to content", async ({
			...
			const contentId = await page
				.getByTestId("collapsible-content")
				.getAttribute("id");
			await expect(page.getByTestId("collapsible-trigger")).toHaveAttribute(
				"aria-controls",
				contentId as string,
		...
		test("should not have `role='region'` on content", async ({
			page,
			renderer,
		}) => {
			await expect(page.getByTestId("collapsible-content")).not.toHaveAttribute(
				"role",
			);
		});
	...
	test.describe("Default State", () => {
		...
		test("should be open by default when `open` prop is true", async ({
			...
			await expect(
				page.getByTestId("open-collapsible-trigger"),
			).toHaveAttribute("aria-expanded", "true");
		});

		test("should allow closing initially open collapsible", async ({
			page,
			renderer,
		}) => {
			await page.goto(`/${renderer}/collapsible/open`);
	...
	test.describe("Keyboard Navigation", () => {
		...
		test("should allow `Tab` into open collapsible content", async ({
			page,
			renderer,
		}) => {
		...
		test("should skip closed collapsible content in tab order", async ({
			page,
			renderer,
		}) => {
			await page.goto(`/${renderer}/collapsible/focusable`);
	...
	test.describe("Mouse Interaction", () => {
		...
		test("should open collapsible when trigger is clicked", async ({
			page,
			renderer,
		}) => {
			const trigger = page.getByTestId("collapsible-trigger");
		...
		test("should close collapsible when trigger is clicked again", async ({
			page,
			renderer,
		}) => {
			const trigger = page.getByTestId("collapsible-trigger");
	...
	test.describe("Focus Management", () => {
		...
		test("should allow focus into content after opening", async ({
			page,
			renderer,
		}) => {
			await page.goto(`/${renderer}/collapsible/focusable`);
	...
	test.describe("Hidden Until Found", () => {
		...
		test("should have `hidden='until-found'` attribute when closed", async ({
			page,
			renderer,
		}) => {
			await expect(page.getByTestId("collapsible-content")).toHaveAttribute(
		...
		test("should remove `hidden` attribute when open", async ({
			page,
			renderer,
		}) => {
			await page.getByTestId("collapsible-trigger").click();
		...
		test("should restore `hidden='until-found'` when closed again", async ({
			page,
			renderer,
		}) => {
			await page.getByTestId("collapsible-trigger").click();
	...
	test.describe("Arrow Key Non-Navigation", () => {
		...
		test("should not navigate when `ArrowUp` is pressed on trigger", async ({
			page,
			renderer,
		}) => {
			await page.goto(`/${renderer}/collapsible/multiple`);
	...
	test.describe("Multiple Collapsibles", () => {
		test("should operate independently of each other", async ({
			page,
			renderer,
		}) => {
			await page.goto(`/${renderer}/collapsible/multiple`);
	...
	test.describe("Nested Collapsibles", () => {
		...
		test("should allow opening outer collapsible", async ({
			page,
			renderer,
		}) => {
		...
		test("should keep inner collapsible state when closing outer", async ({
			page,
			renderer,
		}) => {
			await page.getByTestId("outer-trigger").click();
		...
		test("should operate inner and outer independently", async ({
			page,
			renderer,
		}) => {
			await page.getByTestId("outer-trigger").click();
	...
	test.describe("Separated Trigger and Content", () => {
		...
		test("should toggle content that is in a different DOM subtree", async ({
			page,
			renderer,
		}) => {
		...
		test("should toggle separated content via keyboard", async ({
			page,
			renderer,
		}) => {
			const trigger = page.getByTestId("separated-trigger");
...
test.describe("Dynamic", () => {
	test("should handle conditional render, open prop, remount, multi-instance, props passthrough", async ({
		...
		await expect(page.getByTestId("collapsible-root")).toHaveClass(
			/collapsible-root/,
		);
