...
test.describe("Collapsible", () => {
	...
	test.describe("Initial state", () => {
		test("respects the `open` prop on initial render", async ({
			...
			await expect(
				page.getByTestId("open-collapsible-content"),
			).not.toBeVisible();
		});
	});
	...
	test.describe("Activation", () => {
		...
		for (const key of ["Enter", "Space"] as const) {
			test(`toggles via ${key} and keeps focus on the trigger`, async ({
				page,
			}) => {
				const trigger = page.getByTestId("collapsible-trigger");
				const content = page.getByTestId("collapsible-content");
	...
	test.describe("Keyboard inertness", () => {
		test("Arrow keys do not move focus off the trigger", async ({
			page,
			renderer,
		}) => {
	...
	test.describe("Focus management", () => {
		...
		test("closed panel content is excluded from the tab order", async ({
			page,
		}) => {
			await page.getByTestId("focusable-trigger").focus();
			await page.keyboard.press("Tab");
		...
		test("clicks inside the open panel do not collapse it", async ({
			page,
		}) => {
		...
		test("closing the panel hides content that previously held focus", async ({
			page,
		}) => {
	...
	test.describe("Nested", () => {
		...
		test("inner state survives collapsing and re-opening the outer", async ({
			page,
		}) => {
	...
	test.describe("Scroll prevention", () => {
		test("Space on the trigger does not scroll the page", async ({
			page,
			renderer,
		}) => {
			await page.goto(`/${renderer}/collapsible/basic`);
