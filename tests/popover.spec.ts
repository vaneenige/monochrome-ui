...
test.describe("Popover", () => {
  ...
  test.describe("ARIA Attributes", () => {
    ...
    test("should have `aria-controls` on trigger matching content id", async ({ page }) => {
      const ariaControls = await page.getByTestId("click-trigger").getAttribute("aria-controls")
      const contentId = await page.getByTestId("click-content").getAttribute("id")
      expect(ariaControls).toBe(contentId)
    })

    test("should have `aria-labelledby` on content matching trigger id", async ({ page }) => {
      const triggerId = await page.getByTestId("click-trigger").getAttribute("id")
    ...
    test("should have `aria-hidden='true'` on content when closed", async ({ page }) => {
      await expect(page.getByTestId("click-content")).toHaveAttribute("aria-hidden", "true")
    })

    test("should have `aria-hidden='false'` on content when open", async ({ page }) => {
      await page.getByTestId("click-trigger").click()
      await expect(page.getByTestId("click-content")).toHaveAttribute("aria-hidden", "false")
    })

    test("should have `popover='manual'` on content", async ({ page }) => {
      await expect(page.getByTestId("click-content")).toHaveAttribute("popover", "manual")
    })

    test("should have `type='button'` on trigger", async ({ page }) => {
      await expect(page.getByTestId("click-trigger")).toHaveAttribute("type", "button")
    })
  })
  ...
  test.describe("Click Behavior", () => {
    ...
    test("should allow interactive children to fire their own handlers", async ({ page }) => {
      await page.getByTestId("click-trigger").click()
    ...
    test("should close other popover when opening a new one", async ({ page }) => {
      await page.getByTestId("click-trigger").click()
  ...
  test.describe("Focus Management", () => {
    ...
    test("should return focus to trigger when closed by trigger click", async ({ page }) => {
      await page.getByTestId("click-trigger").click()
      await page.getByTestId("click-trigger").click()
      await expect(page.getByTestId("click-trigger")).toBeFocused()
    })

    test("should tab from content into focusable children", async ({ page }) => {
      await page.getByTestId("click-trigger").click()
      await page.keyboard.press("Tab")
      await expect(page.getByTestId("copy-button")).toBeFocused()
    })

    test("should return focus to trigger via Escape from a focused child", async ({ page }) => {
      await page.getByTestId("click-trigger").click()
  ...
  test.describe("Disabled", () => {
    test("should not open when trigger has `aria-disabled='true'`", async ({ page }) => {
      await page.getByTestId("disabled-trigger").click({ force: true })
      await expect(page.getByTestId("disabled-content")).not.toBeVisible()
    })

    test("should not open when disabled trigger is activated via keyboard", async ({ page }) => {
  ...
  test.describe("Mutual Exclusion", () => {
    test("should close when page is scrolled outside popover", async ({ page }) => {
    ...
    test("should stay open when scrolling a scrollable child inside content", async ({ page }) => {
